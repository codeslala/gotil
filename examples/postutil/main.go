...
import (
	...
	"github.com/codeslala/gotil/oauth2/clientcredentials"
...
func main() {
	...
	// ------------------------------------------------------------------
	// clientcredentials.Config 提供oauth2支持，目前仅支持客户端密码授权。
	cfg := clientcredentials.Config{
		ClientID:     "ID",
		ClientSecret: "secret",
	}

	// postutil可继承clientcredentials.Config.Client()
	// 此时c.Post()等同于cfg.Client().Post()
	c = postutil.ClientInherit(cfg.Client())

	// 接下来的使用方法与上述无异,不再重复介绍。
}
